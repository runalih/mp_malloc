...
int main(int argc, char **argv, char **envp) {
  ...
  for (char **env = envp; *env != 0; env++) {
	...
	const char* library_sideload_lib = NULL;
	if(use_libc_alloc) {
		library_sideload_lib = "lib/mstats-libc-alloc.so";
	} else {
		library_sideload_lib = "lib/mstats-alloc.so";
	}
  ...
  int forkid = 0;
	...
	if(evaluate) {
		...
		if(use_libc_alloc) {
			result_fname = "mstats_result_libc.txt";
		} else {
			result_fname = "mstats_result.txt";
		}
		FILE *result_file = fopen(result_fname,"w+");
		if(result_file) {
			if(result == 0)
				fputs("1\n", result_file);
			else
